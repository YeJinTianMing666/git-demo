...
<div class='sub-sort'>
  ...
  <div class="check">
    <XtxCheckedBox @change="changeCheck" v-model="sortParams.inventory"> 仅显示有货商品</XtxCheckedBox>
    <XtxCheckedBox @change="changeCheck"  v-model="sortParams.onlyDiscount">仅显示特惠商品</XtxCheckedBox>
  </div>
</div>
</template>

<script>
import XtxCheckedBox from './checked-box'
import { reactive } from 'vue'
export default {
  name: 'SubSort',
  components: { XtxCheckedBox },
  setup (props, { emit }) {
    // 1. 根据后台需要的参数定义数据对象
    ...
    const changeSort = (sortField) => {
      if (sortField === 'price') {
        ...
        if (sortParams.sortMethod == null) {
          ...
          sortParams.sortMethod = sortParams.sortMethod === 'desc' ? 'asc' : 'decs'
        }
      } else {
        // 如果排序未改变停止逻辑
